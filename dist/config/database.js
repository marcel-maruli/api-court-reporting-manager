...
exports.seedInitialData = exports.initDatabase = exports.pool = void 0;
...
const initDatabase = async () => {
  ...
  `;
    try {
        ...
        await exports.pool.query(createUsersTableQuery);
...
const seedInitialData = async () => {
    try {
        const checkUsers = await exports.pool.query("SELECT id FROM users LIMIT 1");
      ...
      `);
        ...
        const checkJobs = await exports.pool.query("SELECT id FROM jobs LIMIT 1");
        if (checkJobs.rows.length === 0) {
            const reporters = await exports.pool.query("SELECT id FROM users WHERE role = 'REPORTER' ORDER BY id ASC");
            const editors = await exports.pool.query("SELECT id FROM users WHERE role = 'EDITOR' ORDER BY id ASC");
      ...
      `);
        ...
        const checkPayouts = await exports.pool.query("SELECT id FROM financial_payouts LIMIT 1");
        if (checkPayouts.rows.length === 0) {
            const jobTranscribed = await exports.pool.query("SELECT id, reporter_id FROM jobs WHERE status = 'TRANSCRIBED' LIMIT 1");
            const jobReviewed = await exports.pool.query("SELECT id, reporter_id, editor_id FROM jobs WHERE status = 'REVIEWED' LIMIT 1");
            const jobCompleted = await exports.pool.query("SELECT id, reporter_id, editor_id FROM jobs WHERE status = 'COMPLETED' LIMIT 1");
        ...
        `);
            ...
            console.log('🌱 Seed data for "financial_payouts" successfully inserted.');
        }
    }
    catch (error) {
